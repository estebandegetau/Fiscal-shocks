...
rr_sources <- tribble(
  ...
  "CBO Budget and Economic Outlook", "Non-partisan revenue estimates (post-1974)", "Deferred", "CAPTCHA-protected; revisit in Phase 1",
...
**Phase 0 (Current):** 3 of 9 R&R sources available (ERP, Treasury, Budget). These aggregate sources cover executive and legislative fiscal narratives suitable for codebook development (C1-C4).

**CAPTCHA-Protected Sources:** CBO (cbo.gov) and SSB (ssa.gov) require human verification, blocking automated download. URL functions exist in `R/pull_us.R` and can be re-enabled if access is resolved. Revisit in Phase 1.
...
**Implication:** Our Phase 0 codebooks will be validated using executive-level narratives (ERP, Treasury, Budget). Supplementary sources (CBO, SSB) and per-bill sources can be integrated in Phase 1 if codebook evaluation reveals systematic errors requiring additional coverage.
:::
