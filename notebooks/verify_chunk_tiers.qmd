...
Visual inspection of Tier 1 matches confirms that verbatim passage text appears within the matched chunk. For each sample, we recover the passage from `aligned_data`, locate it in the chunk text, and highlight the match with surrounding context.
...
for (act in sampled_acts_t1) {
  ...
  search_str <- substr(str_squish(tolower(passage)), 1, 100)
  chunk_text_sq <- str_squish(tolower(chunk_row$text))
...
for (act in sampled_acts_t2) {
  ...
  if (act %in% names(COOCCURRENCE_RULES)) {
    ...
    terms <- str_squish(tolower(subs$term))
