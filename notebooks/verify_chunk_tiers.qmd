...
The identification of *known* acts is only implemented in the US, as it is the only country for which we know the acts from @romer2011.
...
# Pre-compute normalized text for matching verification
...
for (i in seq_len(nrow(aligned_data))) {
  ...
  if (act %in% names(COOCCURRENCE_RULES)) {
    ...
    for (rule in COOCCURRENCE_RULES[[act]]) {
      ...
      mechanism_rows[[length(mechanism_rows) + 1]] <- tibble(
...
mechanism_tbl <- bind_rows(mechanism_rows)
```
...
for (i in seq_len(nrow(aligned_data))) {
  ...
  if (any(matched)) {
    timing_data[[length(timing_data) + 1]] <- tibble(
      act_name = act,
      chunk_year = chunks$year[matched]
    )
  }
}
...
# Build regex pattern from all act names (escaped for regex safety)
act_names_pattern <- paste0(
  "(",
  paste(str_replace_all(tolower(aligned_data$act_name), "([.()\\[\\]])", "\\\\\\1"),
        collapse = "|"),
  ")"
)

neg_check <- c1_chunk_data$negatives %>%
  mutate(has_act_name = str_detect(tolower(text), act_names_pattern))

contamination_rate <- mean(neg_check$has_act_name)
n_contaminated <- sum(neg_check$has_act_name)
```

**Contamination Rate**: `r scales::percent(contamination_rate, accuracy = 0.01)` (`r scales::comma(n_contaminated)` / `r scales::comma(nrow(neg_check))` chunks)
...
if (n_contaminated > 0) {
  contaminated <- neg_check %>% filter(has_act_name)
  contaminated %>%
    slice_sample(n = min(5, nrow(contaminated))) %>%
    mutate(text_excerpt = str_trunc(text, 150)) %>%
    select(chunk_id, doc_id, year, text_excerpt) %>%
    gt() %>%
    cols_label(
      chunk_id = "Chunk",
      doc_id = "Doc",
