...
- **C1-step comparability:** distinct C1 measure-name counts per year by language (pre-aggregation drift); carries forward the foreign-comparator contamination + 2018-BM low-recall residuals as C1-level (not aggregation) findings.
- **C0 aggregation diagnostics:** per-doc compression table (does C0 aggregate symmetrically across languages where JW fragmented BM more?); per-language act-count / exo-endo / label-marginal tables; joint cross-language merge rate (does the prompt bridge `Subsidi Bahan Api` ↔ `fuel subsidy`?).
- **Headline — final-output consistency:** act-name-year multiset by language plus **two timeline figures** (`plot_malay_act_timeline()`, timing = act-name year vs. source-doc year) placing each side's acts by year, colour = motivation, shape = sign, solid/faded = exogenous/endogenous, faceted by language. Reader does the paired comparison visually; the "BM over-uses LONG_RUN" signal survives as a label-marginal shift without pairing.
