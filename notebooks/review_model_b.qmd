...
## Root Cause Analysis: Val-Test Performance Gap

This section documents a deep investigation into why Model B shows 90% validation accuracy but only 33.3% test accuracy.

### 1. Data Split Distribution Mismatch

The test set has a different category distribution than training and validation:
...
**Critical issue:** The test set is missing the Countercyclical category entirely. With only 44 total acts and 4 categories, stratified sampling couldn't guarantee representation of all classes in the small test set. This means:

- The model cannot be evaluated on Countercyclical recall in the test set
- The test set over-represents Spending-driven acts (50% vs 32% in training)
- Performance metrics are unstable due to small sample size (6 acts)

### 2. Test Set Contains Systematically Hard Cases

Analysis of the 4 misclassified test acts reveals they share confusing surface features:

**Error Pattern 1: Long-run → Countercyclical (2 acts)**

Both misclassified Long-run acts have "economic improvement" language that resembles countercyclical framing:

- **Public Law 90-26 (1967):** Restoration of Investment Tax Credit after inflation moderated. The timing (following economic concerns) triggers countercyclical pattern matching, but the act's purpose was restoring a structural policy suspended earlier, not stimulus.

- **Revenue Act of 1978:** Capital gains reduction during stagflation. The economic uncertainty of the late 1970s creates countercyclical-looking context, but the capital gains reform was a structural efficiency measure advocated for years.

**Error Pattern 2: Spending-driven → Deficit-driven (2 acts)**

Both misclassified Spending-driven acts use language about program solvency that overlaps with deficit concerns:

- **Social Security Amendments of 1956:** Mentions "self-supporting" and "contribution adjustments" — language shared with Deficit-driven acts — but the primary purpose was funding disability insurance expansion.

- **Social Security Amendments of 1961:** Similarly mentions contribution revision and trust fund solvency, but the act primarily enabled early retirement benefits (spending expansion).

### 3. System Prompt Vulnerabilities

Review of `prompts/model_b_system.txt` identified several weaknesses:

1. **Line 119 bias:** The instruction "If both long-run and deficit concerns → Deficit-driven (more urgent)" privileges deficit classification when features overlap.

2. **Abstract boundary test:** The counterfactual "Would this be enacted regardless of business cycle?" is not operationalized with concrete criteria, leaving the model to pattern-match on surface features.

3. **Few-shot gaps:** The training examples do not include counter-examples showing:
   - Spending-driven acts with solvency language (like Social Security amendments)
   - Long-run acts enacted during economic uncertainty

4. **Confidence miscalibration:** All predictions show 100% confidence, even incorrect ones. The prompt provides no guidance on expressing uncertainty.

### 4. Root Cause: Surface Pattern Matching

The fundamental issue is that the model learned linguistic shortcuts rather than economic logic:
...
The model lacks the economic reasoning to distinguish:

- **Countercyclical:** Temporary response TO the business cycle (stimulus, stabilizers)
- **Long-run:** Structural reform enacted regardless of cycle timing

This distinction requires understanding legislative intent and counterfactual reasoning that surface features cannot capture.

### 5. Implications

**For Phase 0 evaluation:**

- The 33.3% test accuracy should be interpreted with caution given small sample size (n=6) and missing Countercyclical representation
- Consider Leave-One-Out Cross-Validation (LOOCV) for more robust performance estimates (~$3 API cost for 44 acts)

**For Phase 1 deployment:**

- Expect errors on acts with ambiguous surface features
- Implement expert review protocol flagging:
  - Exogenous predictions during recession years
  - Spending legislation with solvency language
  - Acts on the Countercyclical/Long-run boundary

**For prompt improvement (future work):**

- Add concrete decision criteria for boundary cases
- Include counter-examples in few-shot set
- Add economic context (recession indicator) as model input
- Consider chain-of-thought prompting for explicit reasoning

### 6. Recommendations for Future Evaluation

1. **Use LOOCV instead of train/val/test split** — With only 44 labeled acts, fixed splits are too sensitive to which acts land in which set

2. **Report confidence intervals** — Bootstrap the small test set to quantify uncertainty in accuracy estimates

3. **Stratify by difficulty** — Identify hard cases a priori (acts with ambiguous features) and ensure balanced representation

4. **Error analysis before prompt changes** — Document why each error occurred before attempting fixes to avoid overfitting to specific cases
...
```{r loocv-metrics, eval=loocv_available}
...
```{r loocv-not-available, eval=!loocv_available, results='asis'}
cat("⚠️ **LOOCV results not yet available.** Run `tar_make(model_b_loocv_eval)` to generate.\n")
```

**LOOCV Interpretation:**

```{r loocv-interpretation, eval=loocv_available, results='asis'}
...
```{r loocv-per-class, eval=loocv_available}
...
```{r loocv-class-interpretation, eval=loocv_available, results='asis'}
...
```{r loocv-confusion, eval=loocv_available}
...
```{r loocv-error-patterns, eval=loocv_available}
...
```{r loocv-dominant-error, eval=loocv_available, results='asis'}
...
```{r loocv-errors-table, eval=loocv_available}
loocv_eval$error_analysis %>%
...
```{r loocv-summary, eval=loocv_available, results='asis'}
...
### Completed Implementations (2026-01-30)

Based on the recommendations in this notebook, the following implementations have been completed:

#### 1. Baseline Reversion (Commit ce060a8)

The `required_acts` parameter was removed from `_targets.R`, reverting to the original few-shot example generation without forced contrasting examples. Current metrics:

- **Validation:** 90% accuracy, 90% macro F1, 90% exogenous accuracy (n=10)
- **Test:** 33.3% accuracy, 50% macro F1, 33.3% exogenous accuracy (n=6)

#### 2. LOOCV Implementation and Execution ✅

A new file `R/model_b_loocv.R` was created with Leave-One-Out Cross-Validation functions to replace the fragile train/val/test split:

```r
# Core functions:
model_b_loocv()           # Run LOOCV on all 44 acts
evaluate_model_b_loocv()  # Compute metrics with bootstrap CIs
```

New targets added to `_targets.R`:

- `model_b_loocv_results` — LOOCV predictions for all 44 acts
- `model_b_loocv_eval` — Evaluation metrics with 95% bootstrap confidence intervals

**LOOCV Results (executed 2026-01-30):**

| Metric | Value | 95% CI | Target | Status |
|--------|-------|--------|--------|--------|
| Overall Accuracy | 63.6% | [49.9%, 77.3%] | >75% | ❌ FAIL |
| Macro F1 | 63.2% | [47.9%, 77.1%] | >70% | ❌ FAIL |
| Exogenous Precision | 76.2% | — | >90% | ❌ FAIL |
| Exogenous Accuracy | 72.7% | [61.3%, 86.4%] | >85% | ❌ FAIL |

**Key finding:** 16 of 44 acts misclassified. Dominant error pattern is Long-run → Countercyclical (6 errors), indicating the model over-applies countercyclical classification to structural reforms with economic improvement language.

**Actual cost:** ~$3 (44 acts × 5 self-consistency samples, ~29 minutes runtime)

#### 3. Expert Review Protocol (docs/phase_1/expert_review_protocol.md)

A comprehensive expert review protocol was created for Phase 1 Malaysia deployment, including:

- **Flagging rules** (since self-consistency cannot flag errors):
  - Mandatory review: Predicted exogenous + recession year (~10-15%)
  - Secondary review: Countercyclical or Long-run predictions (~20-25%)
  - Auto-accept: All other predictions (~60-65%)
- **Validation form templates** for act identification and motivation classification
- **Agreement metrics** (Cohen's kappa, accuracy by category)
- **Error taxonomy** (5 types: language vs substance, missing context, institutional differences, extraction errors, ambiguous ground truth)
- **Malaysia recession years** for flagging: 1985-86, 1997-98, 2001, 2008-09, 2020

#### 4. Prompt Enhancement Assessment

The current Model B system prompt (`prompts/model_b_system.txt`) already contains:

- Critical warning for recession-era acts with efficiency language (lines 51-77)
- EGTRRA 2001 example showing why it's Countercyclical despite "growth" language
- TRA 1986 contrast showing genuine Long-run reform
- Expert review flagging instructions (lines 129-132)

**Conditional enhancement:** No additional prompt changes needed at this time. The economic context input pathway already exists in `R/model_b_classify_motivation.R` (lines 16-18) for Phase 1 deployment if LOOCV reveals systematic errors requiring it.

### Pending Work

1. ~~**Run LOOCV evaluation**~~ ✅ Complete — See LOOCV Evaluation Results section above

2. ~~**Update this notebook**~~ ✅ Complete — LOOCV results section added with full metrics and error analysis

3. **Assess prompt enhancement need** — LOOCV reveals systematic Long-run → Countercyclical errors (6 of 16 misclassifications). Options:
   - **Option A (Minimal):** Add guidance sentence to system prompt about the pattern
   - **Option B (Economic context):** Pass recession indicators to help distinguish categories
   - **Option C (Accept):** Deploy with current performance + expert review protocol

### Cost Summary

| Component | Status | API Cost |
|-----------|--------|----------|
| Baseline verification | ✅ Done | $0 (cached) |
| LOOCV execution | ✅ Done | ~$3 |
| Expert review protocol | ✅ Done | $0 |
| Prompt enhancement | ⏸️ Optional | ~$1 if implemented |
| **Total** | | **~$3** |
