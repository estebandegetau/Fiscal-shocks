...
**Status**: C1 v0.7.0 S3 GATE ACCEPTED for deployment (iter 33, 2026-05-20, commit 3663a80) — multi-measure schema + per-measure country tag, S1-S3 re-validation complete. S1 PASSED iter 30 (max_tokens=3072 bump); S2 iter 31 diagnostic gates missed within noise; S3 iter 32 diagnostic tests within noise vs v0.6.0; S3 manual audit iter 33 distribution 27A/7B/0C/0D/2E/4F (bias-corrected accuracy 81.8%, Tier 1 recall 80.0%, Tier 2 recall 100.0%). The 2 E errors (text_ids 3, 7 — both Tier 1) are v0.7.0-specific regressions: model applied an overly restrictive "substantive" bar despite the codebook authorizing purpose+scope+magnitude as qualifying. Three codebook gaps flagged for future revision but non-blocking. v0.6.0 baseline (S3 GATE PASSED at iter 28: 31A/6B/0E/3F, bias-corrected recall 100%, precision 83.3%) preserved as historical reference. C2b v0.9.1 FROZEN as C2 deliverable (iter 48, 2026-05-06, commit 2bfdf20): iter 47 manual analysis (24A/2B/0C/0D/2E/11F; bias-corrected exogenous precision 0.833 with CI on n=18 likely containing 0.85 gate; sign accuracy on true-exogenous 0.955 PASSES 0.90 gate; F-cluster cross-version persistence supporting reading-(a) borderline-cases over reading-(b) overfit-anchors) plus iter 48 automated S3 (Tests V/VI/VII overall consistency 0.789; generic-label Δacc 0.034; follows-names 0.974; ablation drops ≤7.7pp accuracy with label_definition the largest single contributor at 5.1pp). Combined reading: model has internalized R&R-style reasoning from pretraining; codebook adds modest calibration. C2a at v0.5.1 (2026-08-05: extraction scoped to the measure named in the `Act:` field, after multi-measure chunks were found to contaminate downstream classification; **S1 re-validated PASS**, iter 17, commit 89ffe81 — all four tests pass on the identical 10-chunk sample, max response volume −77%, contamination visibly fixed on US dev data; joint C2 S2 re-run pending and S3 deferred; v0.6.0 per-evidence `measure_name` attribution is still Step 5 of the 7-step sequencing plan, pending). Three deferred C2 items carried to future work: iter 47's minor revision (causal-link priority + restoration-of-suspended-provision rules), Test V combo 4 partial-exclusion finding (known zero-shot limitation), C4 sign-mapping worked-examples requirement. C3-C4 not yet started (will return as separate downstream codebooks if/when needed). C0 (Act Aggregator) planned — design recorded 2026-05-18.
...
- ✅ `c1_measure_id.yml` — Created (v0.7.0, multi-measure + country tag; S1-S3 re-validation pending)
- ✅ `c2a_extraction.yml` — Created (C2a: per-chunk evidence extraction scoped to the named measure, v0.5.1; v0.6.0 per-evidence `measure_name` attribution planned)
- ✅ `c2b_classification.yml` — Created (C2b: act-level motivation classification, v0.9.1, FROZEN)
- ⏸ `c0_aggregator.yml` — Planned (Step 7 of 2026-05-18 sequencing plan; blocks Phase 2 Malaysia deployment)
- `c3_timing.yml`
- `c4_magnitude.yml`
