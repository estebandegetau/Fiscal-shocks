...
- **Original PDFs** (350 documents): Economic Reports of the President, Treasury Annual Reports, Budget Documents

### Current Bottleneck
- **Docling extraction too slow**: Python subprocess overhead + table parsing = 17+ hours for 350 PDFs on laptop
- **Root causes**:
...
**Architecture:**
```
350 PDF URLs → Trigger Lambda functions (parallel) → Docling extraction → S3 JSON outputs → R reads results
```
...
3. R function `pull_text_lambda()`:
   - Invokes Lambda async for all 350 PDFs
...
**Cost:** ~$6.04 (350 invocations × 4.8 min avg × $0.0000166667/GB-second × 3GB)
...
|-----------|-----------|--------|
| AWS Lambda (350 PDFs) | $6.04 | - |
| S3 + CloudWatch | $1.00 | - |
| Model A API (Act Detection) | $5.87 | - |
| Model B API (Motivation) | $7.14 | - |
| Model C API (Info Extraction) | $3.12 | - |
| Contingency (retries/errors) | $8.00 | - |
| **Total** | **~$30** | - |
...
|------|-------|------------------|
| 1-2 | Cloud PDF Extraction | Lambda deployed, 350 PDFs extracted in <10 min |
...
### Operational
- [ ] PDF extraction < 15 minutes for 350 documents
...
## Multilingual Considerations for SEA Deployment

### Target Languages

Phase 1+ will process government documents in:
| Country | Primary Language | Script | Notes |
|---------|-----------------|--------|-------|
| Malaysia | Malay (Bahasa Malaysia) | Latin | Some documents in English |
| Indonesia | Indonesian (Bahasa Indonesia) | Latin | Similar to Malay |
| Vietnam | Vietnamese | Latin (with diacritics) | Tonal marks critical |
| Thailand | Thai | Thai script | Non-Latin, complex |
| Philippines | Filipino/Tagalog + English | Latin | Often bilingual documents |

### Model Selection Criteria

The LLM must either:
1. **Read natively** in all target languages, OR
2. **Have a translation arm** (translate → process → output)

### Recommended Models

#### Option A: Claude 3.5 Sonnet (Current Choice) ✅ **Recommended**
- **Native multilingual**: Trained on diverse web corpus including SEA languages
- **Strong on**: Malay, Indonesian, Vietnamese, Filipino (Latin scripts)
- **Acceptable on**: Thai (less training data, but functional)
- **No translation step needed**: Direct comprehension reduces error propagation
- **Context window**: 200K tokens handles long government documents

#### Option B: GPT-4o (Backup)
- **Native multilingual**: Similar capabilities to Claude
- **Strong on**: All target languages
- **Vision API**: Better for scanned/image-heavy Thai documents
- **Cost**: ~2x Claude for equivalent tasks

#### Option C: Gemini 1.5 Pro
- **Native multilingual**: Google's multilingual training
- **1M context window**: Useful for very long budget documents
- **Weaker on**: Economic domain knowledge vs Claude/GPT-4

### NOT Recommended

| Model | Issue |
|-------|-------|
| Mistral 7B (local) | Poor SEA language support; would require fine-tuning |
| LLaMA 3 (local) | English-centric; Thai/Vietnamese very weak |
| Translation API → English LLM | Error propagation; loses fiscal terminology nuance |

### Language-Specific Considerations

#### Thai (Highest Risk)
- **Script**: Non-Latin, no word boundaries
- **OCR quality**: May need Thai-specific OCR (Tesseract Thai or Google Vision)
- **Recommendation**: Use Claude's native Thai + Vision API for scanned docs
- **Fallback**: Google Translate API → English → Claude (last resort)

#### Vietnamese
- **Diacritics**: Critical for meaning (e.g., "ma" vs "mà" vs "má")
- **PDF extraction**: Docling handles Unicode well; verify diacritic preservation
- **Recommendation**: Claude native (strong Vietnamese support)

#### Malay/Indonesian
- **Similarity**: ~80% mutual intelligibility; can share few-shot examples
- **English loanwords**: Many fiscal terms borrowed from English
- **Recommendation**: Claude native (excellent support)

#### Filipino
- **Code-switching**: Documents mix Filipino and English
- **Recommendation**: Claude handles bilingual text well

### Implementation Strategy

```r
# In model prompts, add language context
model_b_classify_motivation <- function(act_name, passages_text, year, language = "en") {
  # Adjust system prompt based on document language
  lang_instruction <- switch(language,
    "ms" = "The following passages are in Malay (Bahasa Malaysia).",
    "id" = "The following passages are in Indonesian (Bahasa Indonesia).",
    "vi" = "The following passages are in Vietnamese.",
    "th" = "The following passages are in Thai.",
    "tl" = "The following passages are in Filipino/Tagalog (may include English).",
    "en" = ""
  )

  # Claude processes natively - no translation step
  # ...
}
```
...
Before full SEA deployment:
1. **Test set per language**: 10 documents each with known fiscal acts
2. **Metrics**: Same as US benchmark (F1 > 0.85, accuracy > 0.75)
3. **Error analysis**: Check if errors cluster by language
4. **Thai deep-dive**: Extra validation given script complexity

### Cost Implications

Multilingual processing with Claude 3.5 Sonnet:
- **No additional cost**: Native multilingual, same pricing
- **Potential savings**: No translation API fees
- **Risk**: Thai documents may need more tokens (less efficient encoding)
