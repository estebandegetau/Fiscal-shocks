...
| **Phase 2** | Malaysia Pilot | Deploy codebooks to Malaysia documents (1980-2022) with expert validation | Expert-validated Malaysia fiscal-events dataset (exogeneity-tagged, corpus-linked; assembled in components — tax-side statutory first) |
...
| **C2: Motivation + Sign** | RR3 (sign), RR5 (motivation) | 4-way R&R motivation classification (`SPENDING_DRIVEN`, `COUNTERCYCLICAL`, `DEFICIT_DRIVEN`, `LONG_RUN`); `exogenous` derived as `motivation ∈ {DEFICIT_DRIVEN, LONG_RUN}`; sign of effect on fiscal liabilities | 4-way label + sign + `enacted` | ✅ S3 PASS (v0.9.1 FROZEN at iter 48) — **measured under C2a v0.5.0**; C2a now v0.5.1 (S1 PASS iter 17, joint S2 re-run pending, S3 deferred; see C2 verdict note); C2a v0.6.0 measure-attribution planned |
...
**This section is authoritative on what has been validated.** The methodological paper's §"What is validated, and what is not" (`index.qmd`) is the outward-facing rendering of the criteria and statuses below, written for readers who will not open this document. When the two disagree, this section wins and the paper section is what gets corrected (2026-08-06).
...
| C0: Act Aggregator | Year alignment & fragmentation/spurious rate | Diagnostic | Secondary gate + over/under-merge diagnostics; M5 leading method (selected on Haiku; deployment defaults to Sonnet 4.6 — see C0 Blueprint) |
...
| C2a: Evidence Extraction | Per-act evidence coverage | ≥95% | Share of correctly-identified acts for which C2a returns ≥1 evidence item. Added 2026-08-06 with v0.5.1's named-measure scoping: a scoped extractor that refuses evidence produces a *missing act*, not a C2b error, so no C2b metric above can detect it. Failure mode to watch: the act discussed under a variant name, an acronym, a pronoun, or a Malay rendering rather than the exact `Act:` string |
...
**These numbers were measured under C2a v0.5.0 (2026-08-06).** C2b's own YAML is untouched, so the v0.9.1 freeze stands as a statement about the *classifier*; but the evidence pool feeding it changed when C2a moved to v0.5.1's named-measure scoping. The change is not marginal even on single-act US dev chunks: at iteration 17, on an identical 10-chunk sample, maximum response volume fell 77% and two of ten chunks went to empty evidence sets. The iter-47 manual audit distribution (24A/2B/0C/0D/2E/11F) and the precision/sign figures above therefore describe classifications made on inputs the current pipeline would not reproduce. The joint C2 S2 re-run will re-establish the S2 numbers; **S3 is deferred, so the manual-audit figures carry this caveat until a re-audit retires it.** Read the verdict as *C2b v0.9.1 evaluated under C2a v0.5.0*, not as a property of the pipeline as it stands.
...
The upper bound is also an upper bound on *evidence richness*, not only on bucketing (2026-08-06). It was computed with C2a v0.5.0 sweeping all motivation evidence present in a chunk; v0.5.1 supplies a strictly narrower pool. When the gap is next measured, the two sides must be run under the same C2a version or the difference will conflate aggregator-induced degradation with the scoping change.
...
**Two-stage architecture.** C2 keeps the C2a (per-chunk evidence extraction) → C2b (act-level classification) split. **C2a v0.5.1** is a pure extraction codebook (no `classes` block) that emits `evidence[]` (motivation quotes), `enacted_signals[]`, and `timing_signals[]` per chunk. As of v0.5.1 extraction is **scoped to the measure named in the `Act:` field**: a chunk may discuss several measures, and evidence about the others is out of scope (2026-08-05; see `docs/deltas.md`). The `timing_signals[]` array is preserved on the C2a side for use by the future C3 codebook; C2b v0.9.0 ignores it. **C2b v0.9.0** receives an act_name, year, evidence array, and enacted-status signals; it returns the v0.9.0 schema below. C3 (when built) and C4 (when built) will consume the same C2a evidence pool, reading `timing_signals[]` and motivation/magnitude evidence respectively.
...
**Deployment model (2026-06-09).** Method *selection* was scored on Haiku via RR-recovery (above); at Malaysia deployment scale (877 single-shot surface forms, no ground truth) Haiku fragments high-frequency acts it cannot attend to (GST split 9 ways; EN/BM "Cukai Barang dan Perkhidmatan" ↔ "Goods and Services Tax" not bridged) and commits at least one cross-act over-merge, while **Sonnet 4.6** (streaming path, `run_c0_deployment_stream`) consolidates these with zero detected cross-act over-merges (ARI 0.564; Sonnet's merges nearly nest Haiku's). Under the asymmetric-cost rule (over-merge corrupts C2b = costly; under-merge = recoverable) **deployment defaults to Sonnet** — a capability upgrade on the unchanged v0.2.0 instruction, not a codebook change. Sonnet has not yet been re-scored against the US RR gold; that re-run (plus the Malaysia EN/BM consistency read) is planned soon, with Phase 2 expert review as the ultimate validation backstop. **Deployment QA:** the integrity check (n_returned, parse_ok, end_turn) is necessary but not sufficient as a clustering-quality gate — `guard_c0_merge_rate()` (hard-FAIL < 1% merge rate, WARN < 5%) on the pass-through target `country_c0_clusters_checked` halts before C2b on a degenerate no-merge, a defensive guard against intermittent server-side draws on byte-identical requests (LLM calls send no API seed; temp=0 is not a determinism guarantee). Carried forward: wire the within-cluster year-span over-merge diagnostic as a standing guard.
...
- ✅ `c2a_extraction.yml` — per-chunk motivation, enacted-status, and timing evidence extraction, scoped to the named measure (v0.5.1, 2026-08-05; `timing_signals[]` retained for future C3; v0.6.0 per-evidence `measure_name` attribution still planned 2026-05-18; see C2 Blueprint > Input schema)
...
**Deliverable shape (2026-06, evolving).** The Phase 2 output is being reframed from a single R&R-style tax-shock table to a *composite fiscal-events dataset*: a simple but rich set of Malaysia fiscal events, each (a) tagged exogenous/endogenous via C2 and (b) explicitly attached to the source document corpus. It is assembled in three components, all now scaffolded and Malaysia-frozen (2026-06): (1) tax-side statutory changes by activity taxed (CIT, PIT, VAT/consumption) — 22 events; (2) spending-side fiscal changes (stimulus/relief packages, subsidy-policy changes, large allocations, Five-Year Plan launches) — 14 events; (3) fiscal-holiday / incentive events (tax holidays, investment & reinvestment allowances, concessionary investor rates, zones, sectoral/R&D incentives, consumption holidays, personal reliefs) — 15 events; 51 in total. The relevance-curated C0/C2 act inventory (`notebooks/deployment.qmd`, top acts by an 80%-cumulative `relevance = n_evidence × n_chunks` cut) is the exploratory raw material from which these components are distilled, not the final deliverable. Each component is built by per-component agentic identification skills (frozen to `data/validated/{ISO}_{INSTRUMENT}_shocks.qs` — hand-curated reference *inputs* analogous to `data/raw/us_shocks.csv`, a documented carve-out from the all-data-through-`tar_target` policy; see `CLAUDE.md` Data Generation Policy; read back via the `{tax,spending,incentive}_shock_files` targets), seeded by a validated, genuinely rich C1 first-pass scan that feeds all three components, then human-curated by the analyst.

**Validation of the composite deliverable.** All 51 events have been submitted to **regional fiscal-policy experts for approval** — the primary validation for the whole pipeline, and especially for the agentic carve-out; analyst curation is not the validation step. The ≥80% (C1) / ≥70% (C2) agreement criteria above apply to these component datasets under expert review, not to the full raw inventory. In parallel, the methodological paper is to carry component-specific *statistical* validations: Das et al. (2026) validation procedures for the spending leg and Klemm & Van Parys (2012) for the incentives leg. **Both are outstanding** — neither is present in the current paper draft (`index.qmd`, 2026-08-06).

**The submitted version of the dataset carries a known-defective exogeneity column (2026-08-06).** All three enrichment tails were run before the 2026-08-05 evidence-scoping fix, so every event's C2b label was assigned under the measure-blind join (exposure: 95/100 tax member-chunk rows, 109/109 spending, 40/41 incentive) and four labels are known wrong (MY-CIT-08, MY-CIT-09, MY-PIT-01, MY-PIT-06). Expert reviewers are therefore reading a column that must be re-derived. Until the tails are re-run and the corrected rows re-audited, the in-flight review validates the **event inventory** (what the components identified, their dates, instruments and magnitudes) but **not** the motivation/exogeneity labels attached to it. Whether the label review restarts on the corrected dataset or is treated as a first pass to be diffed against it is an open decision.

**Known limitations (acknowledged; fixes deferred, all pending expert + statistical validation).** (i) Magnitude and timing fields are extracted agentically (standing in for the unbuilt C3/C4) and are **not** H&K-validated — an external rate anchor (Vegh & Vuletin 2015 / OECD) is flagged as future work. (ii) The spending leg is classified by the tax-validated C2b, but R&R's `SPENDING_DRIVEN` motivation is defined for *tax* actions financing spending, not for spending actions themselves; C2b performs well on spending acts empirically, but the theoretically proper step is a Das et al. (2026)-style spending-motivation prompt (on the to-do list). (iii) The incentive / tax / spending datasets are disjoint by construction, but the boundary for a borderline measure is a per-event reviewer adjudication still pending expert and statistical validation.
