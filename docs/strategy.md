...
**Deliverable shape (2026-06, evolving).** The Phase 2 output is being reframed from a single R&R-style tax-shock table to a *composite fiscal-events dataset*: a simple but rich set of Malaysia fiscal events, each (a) tagged exogenous/endogenous via C2 and (b) explicitly attached to the source document corpus. It is assembled in components — (1) tax-side statutory changes by activity taxed (CIT, PIT, VAT/consumption) — *underway*, 22 events frozen for Malaysia 2026-06; (2) spending-side fiscal changes — *next*; (3) fiscal-holiday / incentive events — *later*. The relevance-curated C0/C2 act inventory (`notebooks/deployment.qmd`, top acts by an 80%-cumulative `relevance = n_evidence × n_chunks` cut) is the exploratory raw material from which these components are distilled, not the final deliverable. The tax-side component is built by per-instrument agentic identification skills (frozen to `data/validated/{ISO}_{INSTRUMENT}_shocks.qs` — hand-curated reference *inputs* analogous to `data/raw/us_shocks.csv`, a documented carve-out from the all-data-through-`tar_target` policy; see `CLAUDE.md` Data Generation Policy; read back via the `tax_shock_files` target), human-curated by the analyst and then submitted to **regional fiscal-policy experts for approval** before downstream use — analyst curation is not the validation step. The ≥80% (C1) / ≥70% (C2) agreement criteria above apply to the component datasets under expert review, not to the full raw inventory. Caveat: the tax-side component's magnitude and timing fields are extracted agentically (standing in for the unbuilt C3/C4) and are **not** H&K-validated — an external rate anchor (Vegh & Vuletin 2015 / OECD) is flagged as future work.
