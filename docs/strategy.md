...
**Validation of the composite deliverable.** All 51 events have been submitted to **regional fiscal-policy experts for approval** — the primary validation for the whole pipeline, and especially for the agentic carve-out; analyst curation is not the validation step. The ≥80% (C1) / ≥70% (C2) agreement criteria above apply to these component datasets under expert review, not to the full raw inventory. In parallel, the methodological paper carries component-specific *statistical* validations: Das et al. (2026) validation procedures for the spending leg and Klemm & Van Parys (2012) for the incentives leg.
