...
**Deliverable (2026-06, evolving).** The Malaysia output is a *composite fiscal-events dataset* — exogeneity-tagged, corpus-linked events — assembled in three components, all now scaffolded and frozen (2026-06): tax-side statutory (CIT/PIT/VAT, 22 events) + spending-side (14 events) + fiscal incentives/holidays (15 events); 51 in total, each seeded by a validated, genuinely rich C1 first-pass scan. The relevance-curated C0/C2 inventory is exploratory raw material, not the deliverable. All 51 events have been submitted for **regional-expert approval** — the primary validation, especially of the agentic carve-out; the agreement criteria above apply to these component datasets, not the full raw inventory. The methodological paper adds component-specific statistical validation: Das et al. (2026) procedures for the spending leg and Klemm & Van Parys (2012) for incentives. Known limitations carried forward (pending expert + statistical validation): the spending leg is labelled by the tax-validated C2b pending a Das et al. (2026)-style spending-motivation prompt, and the incentive / tax / spending disjoint-set boundary is a per-event reviewer adjudication.
