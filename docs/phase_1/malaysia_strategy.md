...
**Core Challenge**: The Phase 0 plan assumed 126 acts and recommended "100+ acts for Malaysia" - both infeasible.

## Strategic Options

### Option 1: Cross-Country Transfer Learning (Recommended)

**Approach**: Train on US data, deploy to Malaysia with expert validation loop

**Implementation**:
1. **Phase 0 (Current)**: Train Models A/B/C on 44 US acts
2. **Phase 1**: Deploy to Malaysia documents (1980-2022)
3. **Validation**: Expert reviews LLM outputs, provides corrections
4. **Iteration**: Use expert feedback to refine prompts/examples
...
**Approach**: LLM generates candidates, expert labels subset, retrain

**Implementation**:
1. Deploy Models A/B/C to Malaysia documents
2. Expert labels top 20-30 LLM-identified acts (high confidence)
3. Fine-tune models on Malaysia examples
4. Re-deploy and validate

**Advantages**:
- Creates Malaysia-specific training data
- Model adapts to local language/patterns
- Expert focuses on highest-value labeling

**Risks**:
- Requires expert time upfront (labeling 20-30 acts = 40-60 hours)
- May not improve over few-shot if sample too small
- Delays Phase 1 timeline

**Success Metrics**:
- Same as Option 1, plus:
- Model performance improvement after fine-tuning (∆F1)
...
2. Run Models A/B/C with US few-shot examples
...
### Revised Framing:
> "Phase 0 trains few-shot models on 44 US fiscal acts with Romer & Romer labels. Phase 1 tests cross-country transfer to Malaysia (1980-2022) with expert validation, demonstrating methodology generalizability despite limited training data."

**Emphasis Shift**:
- FROM: "Scale" (100+ acts per country)
- TO: "Transfer learning" (train US, deploy elsewhere)
...
3. **Update Documents**:
   ...
   - `docs/phase_0/plan_phase0.md`: Add "Malaysia Strategy" section
   - Create `docs/phase_1/plan_phase1.md` with chosen option
...
**Key Message**: "We're not building 100-act datasets for every country. We're demonstrating that LLMs trained on limited US data can assist experts in identifying fiscal shocks cross-country, reducing manual effort from months to weeks."
...
- [ ] Deploy Models A/B/C with US prompts
- [ ] Generate candidate act dataset
...
| Model A/B/C Deployment (20-40 acts × 3 models) | $8-12 |
...
| Models fail to transfer | Low | Critical | This validates research question! Document failure modes |
...
1. We demonstrate LLMs can assist (not replace) experts in cross-country fiscal shock identification
2. Expert validation shows ≥80% agreement on act identification
3. We honestly document where the method works and where it fails
