...
`A` = auto-fetched direct PDF; `R` = manually downloaded and ready; `P` = pending (confirmed unavailable digitally).
...
- **`auto`** — `pdf_url` is a verified direct PDF URL that the deployment pipeline fetches via `urllib`. Tested with Python `urllib.request.urlretrieve` and Tesseract OCR fallback. Only viable for sites without anti-bot infrastructure (PRIHATIN/PENJANA at PMO are the only Malaysian examples).
...
| Crisis booklets | pmo.gov.my (PRIHATIN, PENJANA direct PDFs); pmo.gov.my speech archive (PERMAI/PEMERKASA/PEMULIH transcripts); search for NRP and NERP | Yale eliScholar for NERP-era documents |
