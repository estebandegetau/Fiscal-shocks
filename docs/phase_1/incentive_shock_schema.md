...
| `recovered_chunks` | **list-col** | tibble{`doc_id` chr, `chunk_id` int}; subset of `member_chunks` that C1 did **not** surface. For incentives this is expected to be **most** chunks (C1 is tax-rate-scoped, not incentive-scoped). May be empty. Purely documentary — C2a now runs on every member chunk regardless. |
