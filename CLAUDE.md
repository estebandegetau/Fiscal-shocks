...
- **Methodological paper** (`index.qmd`, "Scaling the Narrative Identification of Fiscal Changes using LLMs"): DRAFT — scaffolded 2026-08-05 (commit 654e7a8), built out to a full body 2026-08-06 (commit 5c747da). Sections present: the pipeline and what it produces, how the Malaysia dataset was built (corpus / C1-as-seed / agentic identification + recall audit / why automated act clustering was rejected / the Aug-2026 evidence-contamination defect written up as an H&K result), Validation I — the US benchmark (C1, C2), Validation II — Malaysia, what is validated and what is not, a procyclicality use case, limitations, and a codebook + recall-audit appendix. Renders to HTML and `academic-typst` (PDF is the primary review artifact). **Deferred:** intro, abstract, conclusion, the procyclicality figure. **Known stale:** the Malaysia exhibits are the June freeze, so the four labels corrected by commits 523d5e7 / 3bce179 are still wrong in the figures (flagged in-document by the draft-status callout and §4.5). Paper-bound exhibits live in `R/iteration_reporting.R`, `R/parse_iteration_logs.R`, `R/pipeline_diagram.R`, `R/dataset_summary.R`, `R/recall_scorecard.R`, `R/codebook_display.R`; three parser/plot defects that were suppressing real data were fixed in 5c747da (Test IV `test_iv_diagnostics` sibling-key dialect, c2b's bias-corrected dialect, and `plot_s3_behavioral()`'s `^V` regex matching VI/VII).
...
- `index.qmd` - The methodological paper (draft; HTML + `academic-typst`). Read-only over the pipeline (`tar_read()` never `tar_make()`); exhibits come from `R/iteration_reporting.R`, `R/parse_iteration_logs.R`, `R/pipeline_diagram.R`, `R/dataset_summary.R`, `R/recall_scorecard.R`, `R/codebook_display.R`
