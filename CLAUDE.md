...
- **Phase 0**: IN PROGRESS — Codebook development using C1-C4 framework
  ...
  - **C2 (Motivation)**: S3 MANUAL ANALYSIS COMPLETE (v0.9.1, iter 47, 2026-05-06, commit 93e4d1c). 39 acts inspected: 24A / 2B / 0C / 0D / 2E / 11F. Bias-corrected exogenous precision 0.833 (raw 0.800; 1.7pp below 0.85 gate but CI on n=18 likely contains gate); sign accuracy on true-exogenous 0.955 PASSES 0.90 gate. Two B-category acts (#4 EGTRRA, #37 Tax Reform 1976) reflect evaluation-framework gap: gold codings for multi-motivation or multi-quarter opposite-signed acts use out-of-codebook conventions our single-label/single-sign output schema cannot evaluate. Two E-category acts point to actionable codebook revisions: #9 (1954 Expiration) shows v0.7.0 rewrite lost the explicit causal-link priority rule, #17 (ITC Suspension) shows sign-extraction failure on suspend-credit semantics (deferred to C4 development with worked sign-mapping example). The 11 F-cluster acts persist across 4 codebook versions and 2 model generations (8/9 iter-35 F-cluster acts remain F in v0.9.1) — decisive evidence that the iter-46 evidence-shuffle "fail (overfit)" verdict (F-A gap -0.333) reflects R&R-judgment-call borderline cases (LR↔CC on Revenue Acts 1964/1971/1978/1977-Reduction; CC↔SPENDING on Tax Adjustment 1966 and Crude Oil 1980) rather than memorization-overfit. Two-codebook architecture: c2a_extraction.yml (v0.4.0, unchanged) + c2b_classification.yml (v0.9.1). Recommendation in iter 47 log: (a) freeze v0.9.1 with documented caveats + (b) minor revision restoring 2 lost rules (causal-link priority for SD; restoration-of-suspended-provision for LR) + (d) document C4 sign-mapping requirement. Awaiting user decision.
