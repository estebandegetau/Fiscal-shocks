...
- **Statutory tax-shock identification layer** (deliverable atop deployment): per-instrument agentic identification skills (`identify-cit`/`identify-pit`/`identify-vat` + `identify-tax-shocks` orchestrator) freeze hand-curated `data/validated/{ISO}_{INSTRUMENT}_shocks.qs` reference datasets (one row per announced act × tax type; contract `docs/phase_1/tax_shock_schema.md`; `tar_target` data-policy carve-out, `docs/deltas.md` 2026-06-25). **Malaysia seeded (2026-06):** CIT 9, PIT 9, CONSUMPTION 4 events frozen (commits 2907f22, d16b896, 1b5c853); provenance notebooks `notebooks/{cit,pit,vat}_identification.qmd`. The bound deliverable with C2a/C2b motivation+exogeneity enrichment (`notebooks/tax_shocks.qmd`, `R/tax_shock_dataset.R`) is wired and API-gated — not yet run.
