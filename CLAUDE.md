...
- **Phase 0**: IN PROGRESS — Codebook development using C1-C4 framework
  ...
  - **C2 (Motivation)**: S3 manual analysis COMPLETE on Haiku v0.5.0 (iteration 30). Two-codebook architecture: c2a_extraction.yml (v0.4.0) + c2b_classification.yml (v0.5.0, label-agnostic instructions). C2b S1 PASS on Haiku (iter 26: 4/4 tests, kappa=1.0). C2b S2: wF1 0.62 [0.45, 0.78] primary / 0.64 sensitivity, Exogenous Precision 0.81 [0.60, 1.00] primary / 0.83 sensitivity (both below targets but stable across iterations and codebook versions). C2b S3 (iter 29): Test VII label-swap bug FIXED — follows_definitions_rate 0.865 / follows_names_rate 0.081 (vs prior 0.205/0.769 on v0.4.0). Ablation: individual components non-load-bearing (Δ wF1 < 0.02 per component), only all-removed drops by 0.057. Manual analysis (iter 30): 24A/0B/4C/0D/1E/9F, bias-corrected accuracy 70.6% (vs 71.4% Sonnet v0.4.0). Residual F-cluster (9 acts, 23.7%) is codebook rule-priority gaps (DR1 vs DR4, DR1 vs BCR4, DR3 multi-dominant tiebreaker, BCR3 scope, SPENDING_DRIVEN scope, return-to-normal misapplication). Decision pending: freeze v0.5.0 and proceed to interpretation, or attempt v0.6.0 multi-component overhaul of decision rules + boundary case rules.
