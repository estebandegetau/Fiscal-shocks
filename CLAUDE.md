...
- **Phase 0**: IN PROGRESS — Codebook development using C1-C4 framework
  ...
  - **C2 (Motivation)**: v0.9.0 S0 COMPLETE — 4-class R&R-anchored rewrite (iter 43, 2026-05-05, commit 8c092af) restoring R&R's four motivation categories (SPENDING_DRIVEN, COUNTERCYCLICAL, DEFICIT_DRIVEN, LONG_RUN). `exogenous` becomes a derived metric (motivation ∈ {DEFICIT_DRIVEN, LONG_RUN}) rather than a codebook output. Two-codebook architecture retained: c2a_extraction.yml (v0.4.0, unchanged) + c2b_classification.yml (v0.9.0). Sign and `enacted` fields retained from v0.8.0 (sign accuracy 0.957 PASS at iter 42); timing surface (`enacted_quarter[]`) removed from C2b (will return as a separate downstream codebook). Drafted by a fresh agent given only R&R primary sources and C1 as a structural template — no access to prior C2b versions, to avoid anchoring on failed designs. Background: investigation found v0.8.0's 0.500 exogenous precision is v0.7.0 reproduced verbatim — the real regression is v0.6.x → v0.7.0 (dropping `classes` lost 31pp precision and made S3 ablation impossible). v0.9.0 attempts to recover 4-way structure without re-introducing v0.6.x's overfit (iter 36 F–A gap −0.333) by holding to three constraints: R&R-language fidelity, country-agnostic vocabulary, and embedded-guardrail design (no detached rule paragraphs). Two specific guardrails embedded inside class bodies target prior failure modes — LONG_RUN: timing-of-decision rule + Das clarifier (prevents macro-context-→-endogenous heuristic, target was 1/15 LONG_RUN recall in v0.8.0); SPENDING_DRIVEN: 1-year temporal rule + structural-framing-of-spending guardrail (target was 5/10 SPENDING_DRIVEN false-positives in v0.8.0). Headline metrics unchanged: {Exogenous Precision ≥85% (binary, derived), Sign Accuracy on True-Exogenous ≥90%}. Floor to beat: 0.500 (v0.7.0/v0.8.0); ceiling to approach: 0.812 (v0.6.1, but inflated by overfit). S3 Tests V–VII now runnable. Pending S1 → S2 → evidence-shuffle diagnostic (gating overfit check, F–A gap > −0.10) → S3.
