...
- **Phase 0**: IN PROGRESS — Codebook development using C1-C4 framework
  ...
  - **C2 (Motivation)**: v0.7.0 redesign IN PROGRESS — minimal Das-et-al.-adapted prompt replacing v0.6.1 (iter 37, 2026-05-01). Two-codebook architecture retained: c2a_extraction.yml (v0.4.0, unchanged) + c2b_classification.yml (v0.7.0, no `classes` block, output schema `{enacted, exogenous, sign, confidence, reasoning}`). Background: iter 35 diagnosed wF1 0.66 as a structural ceiling under any v0.5–v0.6.x rule density on Haiku; iter 36's evidence-shuffle leakage diagnostic produced an F–A median-stability gap of −0.333 (overfit signal at the pre-registered −0.10 threshold). v0.7.0 drops DR1–DR4, BCR1–BCR4, the 4 class definitions, the motivations[] array, and the share/dominant tiebreaker; adds Das et al.'s critical clarifier ("Acknowledging current conditions does not by itself imply endogeneity if the stated motive is explicitly non-cyclical") and a conservative aggregation rule for mixed-component acts. Headline metrics shift from {wF1, Exogenous Precision} to {Exogenous Precision ≥85%, Sign Accuracy on True-Exogenous ≥90%} — see docs/strategy.md C2 Blueprint. Pending S2 zero-shot run on the same 39-act test set; v0.6.1 baseline (iter 33: exogenous precision 0.81 primary / 0.83 sensitivity) is the floor. S3 Tests V–VII guard cleanly when codebook has no classes; the evidence-shuffle diagnostic supersedes them.
...
- C2 (Motivation, v0.7.0): Exogenous Precision ≥85% (binary), Sign Accuracy on True-Exogenous ≥90%
