...
- **Composite fiscal-events identification layer** (deliverable atop deployment): per-component agentic identification skills freeze hand-curated `data/validated/{ISO}_{INSTRUMENT}_shocks.qs` reference datasets (one row per announced act; `tar_target` data-policy carve-out, `docs/deltas.md` 2026-06-25). Three components, each a parallel/hybrid contract + provenance notebook + a 5-target tail in `_targets.R` that reuses the generic C2a/C2b enrichment (`assemble_shock_evidence`/`run_c2b_on_shocks`/`assemble_tax_shock_deliverable`), differing only in the `bind_*` step:
  - **(1) Statutory tax** (`identify-cit`/`identify-pit`/`identify-vat` + `identify-tax-shocks` orchestrator; contract `docs/phase_1/tax_shock_schema.md`). **Malaysia frozen (2026-06):** CIT 9, PIT 9, CONSUMPTION 4 events (commits 2907f22, d16b896, 1b5c853); notebooks `notebooks/{cit,pit,vat}_identification.qmd`. Bound deliverable `notebooks/tax_shocks.qmd` (`R/tax_shock_dataset.R`).
  - **(2) Spending** (`identify-spending`; parallel contract `docs/phase_1/spending_shock_schema.md` — `instrument_type="Expenditure"`, rate fields NA, `spending_category` Das component families; preliminary exogeneity via the Das et al. (2026) two-condition screen). **Malaysia frozen (2026-06-29):** 14 events (commit 0d90234); notebook `notebooks/spending_identification.qmd`, `R/spending_shock_dataset.R`.
  - **(3) Incentives** (`identify-incentives`; hybrid contract `docs/phase_1/incentive_shock_schema.md`). **Malaysia frozen (2026-06-30):** 15 events (commit 0674484); notebook `notebooks/incentives_identification.qmd`, `R/incentive_shock_dataset.R`.
  - Reviewer-facing landing page `notebooks/malaysia_dataset.qmd` presents the bound tax + spending + incentive deliverables. All three enrichment tails are API-gated; all three have been run once (tax 2026-06-26, spending 2026-06-29, incentives 2026-06-30) and **must be re-run** after the 2026-08-05 evidence-scoping fix below.
  - **Evidence-contamination fix (2026-08-05, commits 523d5e7 + 3bce179).** Reviewer feedback on two CIT rows exposed a machinery defect, not a judgement call. `assemble_shock_evidence()` joined shocks to `country_c2a_evidence` on `(doc_id, chunk_id)` alone, but that store holds one evidence row per chunk extracted under whichever measure `filter_c1_measures()` ranked first (`measure_rank == 1L`, discarding 84.5% of C1's measures). The `identify-*` skills cite a chunk because the act's sentence is in it and are not rank-1 restricted, so shocks silently inherited another act's motivation evidence (95/100 tax member-chunk rows, 109/109 spending, 40/41 incentive). A second mechanism compounded it: C2a v0.5.0 extracted **all** motivation evidence in a chunk regardless of the named act, which is harmless on single-act US dev chunks but not on Malaysian budget speeches (median 8 measures per chunk, max 42). Four wrong C2b classifications resulted — MY-CIT-08 (classified on GST evidence, `c2b_sign` `+` on a Cut), MY-CIT-09 (classified on MTRS boilerplate), MY-PIT-06 (classified on GST/oil-price evidence), MY-PIT-01 (chunk-wide over-extraction). Fixes: C2a **v0.5.1** scopes extraction to the measure named in the `Act:` field (new multi-measure worked example); `assemble_shock_evidence()` now runs C2a per `(shock_id, doc_id, chunk_id)` triple scoped to each shock's `act_label` and no longer consumes `country_c2a_evidence`. Verified not to invalidate `country_measure_pool` / `country_c0_clusters` / `country_c0_acts`, so the fragile single-shot Sonnet C0 clustering and its merge-rate guard are untouched. Re-validation scope: S1 + joint C2 S2 (S3 deferred). **C2a S1 re-validated PASS** (iter 17, 2026-08-05, commit 89ffe81): all four tests pass on the identical 10-chunk sample, max response volume −77%, and the contamination is visibly fixed on US dev data (chunk 1, Tax Reform Act of 1969, drops from an off-target 17-item sweep to a correctly empty evidence set). Joint C2 S2 re-run still pending; the tax / spending / incentive enrichment tails must be re-run after it. The `measure_rank == 1L` recall gap is **not** addressed here — that remains Step 5. See `docs/deltas.md` 2026-08-05.
...
- **Methodological paper** (`index.qmd`, "Call it Exogenous: Scaling the Narrative Identification of Fiscal Changes using LLMs"): COMPLETE FIRST DRAFT — scaffolded 2026-08-05 (commit 654e7a8), full body 2026-08-06 (commit 5c747da), abstract + conclusion + procyclicality use case + external rate benchmark 2026-08-17. Sections: the pipeline and what it produces, how the Malaysia dataset was built (corpus / C1-as-seed / agentic identification + recall audit / why automated act clustering was rejected / the Aug-2026 evidence-contamination defect written up as an H&K result), Validation I — the US benchmark (C1, C2), Validation II — Malaysia (EN/BM consistency, worked disagreement cases, **the Végh–Vuletin statutory-rate benchmark**, expert protocol), what is validated and what is not, the procyclicality decomposition, conclusion, limitations, and a codebook + recall-audit appendix. Renders to HTML and `academic-typst` (PDF is the primary review artifact). **Known stale:** the Malaysia exhibits are the June freeze, so the four labels corrected by commits 523d5e7 / 3bce179 are still wrong in the figures — this now has a quantitative consequence, since the decomposition's fragility index is 4 (flagged in-document by the draft-status callout, §4.5 and §7.4). The external benchmark uses the @vegh_how_2015 statutory-rate columns of `data/raw/Full_sample_filtered.dta` (provenance confirmed 2026-08-17; see `docs/data_sources.md`, which also records that the file's derived `change_*` columns are cut-only and must not be used as first differences). Paper-bound exhibits live in `R/iteration_reporting.R`, `R/parse_iteration_logs.R`, `R/pipeline_diagram.R`, `R/dataset_summary.R`, `R/recall_scorecard.R`, `R/codebook_display.R`; three parser/plot defects that were suppressing real data were fixed in 5c747da (Test IV `test_iv_diagnostics` sibling-key dialect, c2b's bias-corrected dialect, and `plot_s3_behavioral()`'s `^V` regex matching VI/VII).
...
See `docs/strategy.md` for authoritative methodology, `docs/two_pager.qmd` for project description, `index.qmd` for the methodological paper draft, and `docs/phase_1/malaysia_strategy.md` for Phase 2 (Malaysia) strategic plan.
...
Key targets: `us_urls`, `us_text`, `us_body`, `aligned_data`, `chunks`, `c1_chunk_data`, `c1_s2_test_set`, `c1_s2_results`, `c1_s2_eval`, `c1_s3_test_set`, `c1_s3_results`, `c2_input_file`, `c2_input_data`, `c2a_s1_results`, `c2b_s1_results`, `c2_s2_results`, `c2_s2_eval`, `c2_s3_results`, `macro_panel`, `my_cycle`, `procyclicality_decomposition`, `rate_path_concordance`
...
- `index.qmd` - The methodological paper (draft; HTML + `academic-typst`). Read-only over the pipeline (`tar_read()` never `tar_make()`); exhibits come from `R/iteration_reporting.R`, `R/parse_iteration_logs.R`, `R/pipeline_diagram.R`, `R/dataset_summary.R`, `R/recall_scorecard.R`, `R/codebook_display.R`, `R/procyclicality.R`
- `docs/data_sources.md` - External reference inputs not produced by the pipeline: what each is, which columns are used, verified data-quality findings, and unconfirmed provenance
...
- `data/validated/` - Frozen artifacts, two kinds: (1) reproducible cached pipeline *outputs* (e.g., `c2_input_data.qs`) decoupling cross-codebook dependencies; (2) human-curated reference *inputs* with no generating target (`{ISO}_{INSTRUMENT}_shocks.qs` from the agentic identify-* skills — see Data Generation Policy carve-out)
...
**Carve-out — human-curated reference inputs (not pure functions).** A small class of artifacts is *authored by a human judgment pass*, not derived by a reproducible function, and so legitimately lives outside a `tar_target` command — exactly like `data/raw/us_shocks.csv` / `us_labels.csv`. The statutory tax-shock datasets `data/validated/{ISO}_{INSTRUMENT}_shocks.qs` (frozen by the agentic `identify-cit`/`identify-pit`/`identify-vat` skills, human-stamped with a recall scorecard) are such inputs: the pipeline *consumes* them via the `tax_shock_files` target, it does not generate them. Their lineage lives in the provenance notebooks (`notebooks/{cit,pit,vat}_identification.qmd`) + the reviewer stamp + the `docs/phase_1/tax_shock_schema.md` contract, not in the targets graph — do not "fix" the missing generating target. So `data/validated/` now holds **two** kinds of frozen artifact: (1) reproducible cached pipeline *outputs* (e.g. `c2_input_data.qs`), which a target must regenerate, and (2) human-curated reference *inputs* (the `*_shocks.qs` files), which by construction have no generating target. The carve-out applies only to category (2).
